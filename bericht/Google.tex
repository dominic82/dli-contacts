...
Die Authentifizierung wird für ein \code{ContacsService}-Objekt, wie in Listing
...
\javalstset{Kontaktsuche per \code{Query}}{lst:searchQuery}
\begin{lstlisting}
...
\javalstset{Kontakt-Objekt (\code{ContactEntry}) erstellen}{lst:createContact}
\begin{lstlisting}
...
\javalstset{Namen in ein Kontakt-Objekt (\code{ContactEntry}) einfügen}{lst:ccsetname}
\begin{lstlisting}
// Name
Name name = new Name();
...
\javalstset{Benutzerdefinierte Einträge zu einem Kontakt-Objekt (\code{ContactEntry}) hinzufügen}{lst:cccustomEntry}
\begin{lstlisting}	
// Firma
...
\javalstset{Das Kontakt-Objekt (\code{ContactEntry}) an den Service übergeben}{lst:ccsendcontact}
\begin{lstlisting}
// Kontakt senden		
